...
async function _migrateProgress(supabase, userId) {
  ...
  if (window.__ONSET_DEV__) console.log('[migration] progress migrated to cloud');
}
...
async function _migrateHotCues(supabase, userId) {
  ...
  if (error) throw error;
  if (window.__ONSET_DEV__) console.log(`[migration] ${rows.length} hot cue track(s) migrated to cloud`);
}
...
async function _migrateSpacedRepetition(supabase, userId) {
  ...
  if (error) throw error;
  if (window.__ONSET_DEV__) console.log('[migration] spaced repetition data migrated to cloud');
}
