...
export class MixerBridge {
  ...
  /**
   * Register a listener and track it for cleanup in destroy().
  ...
  destroy() {
    ...
    // Remove all deck and MIDI router listeners
    if (this._listeners) {
  ...
  _subscribeToMIDI() {
    ...
    this._on(router, 'deck-a:cue', (e) => {
      if (/** @type {CustomEvent} */ (e).detail.value === 1) {
        this._decks.A.cue();
        this._state.set('A', 'isPlaying', false, 'midi');
      }
    });
    this._on(router, 'deck-b:cue', (e) => {
      if (/** @type {CustomEvent} */ (e).detail.value === 1) {
        this._decks.B.cue();
    ...
    this._on(router, 'deck-a:filter', (e) => {
      this._state.set('A', 'filter', /** @type {CustomEvent} */ (e).detail.value, 'midi');
    });
    this._on(router, 'deck-b:filter', (e) => {
