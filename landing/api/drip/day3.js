/**
 * Drip Email — Day 3
 * Subject: "Your first mix — tips for nailing Lesson 1"
 *
 * Triggered daily by Vercel Cron (see landing/vercel.json).
 * Queries the Resend audience for contacts created exactly 3 days ago
 * and sends them the Day 3 tips email.
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Filter to contacts who signed up exactly 3 days ago and haven't unsubscribed
    const targets = (contacts || []).filter(
      (c) => !c.unsubscribed && c.created_at && isNDaysAgo(c.created_at, 3)
    );

    if (targets.length === 0) {
      return res.status(200).json({ sent: 0, message: 'No contacts due for Day 3 email' });
    }

    let html = loadEmailTemplate('email2-tips.html');
    ...
    console.log(`Day 3 drip: sent=${sent} failed=${failed}`);
    return res.status(200).json({ sent, failed });
  } catch (err) {
    console.error('Day 3 drip error:', err);
